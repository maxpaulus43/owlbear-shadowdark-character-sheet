<script lang="ts">
  import { calculateTitleForPlayer, levelUpPlayer, PlayerCharacterStore as pc } from "./lib/model/PlayerCharacter";
  ...
  // --- SYNC IMPORTS ---
  import { initSync, initialSyncComplete, showReauthModal, showConnectionErrorModal, showOfflineConfirmationModal, showConflictModal, conflictedSlots, resolveConflict, syncMessage, login, enableOfflineMode, logout, showSetupModal } from "./lib/services/SyncManager";
  import SyncButton from "./lib/components/SyncButton.svelte";
  import SyncSetupModal from "./lib/components/SyncSetupModal.svelte";
  import SyncModalsContainer from "./lib/components/SyncModalsContainer.svelte";
  // --------------------
      ...
      <div id="sheet" class="bg-black min-w-[277px] max-w-[1000px] p-2 flex flex-wrap gap-2">
        ...
        <div class="flex-[2] min-w-[257px] h-auto min-[805px]:h-[700px] grid grid-rows-8 grid-cols-2 gap-2">
          <div class="col-span-full cell">
            <label
              ><h2>NAME</h2>
              <input type="text" bind:value={$pc.name} /></label
            >
          </div>
          ...
          <div class="cell">
            <label
              ><h2>LEVEL</h2>
              <input type="number" inputmode="numeric" bind:value={$pc.level} max="10" min="1" /></label
            >
          </div>
          <div class="cell">
            <h2>XP</h2>
            <div class="sheet-stat flex gap-1">
              {#if $pc.level < 10}
                <input id="xp" type="number" inputmode="numeric" min="0" bind:value={$pc.xp} />/
                <div>{xpCap}</div>
              {:else}MAX LEVEL{/if}
              <button
                class="text-2xl"
...
{#if $showReauthModal || $showConnectionErrorModal || $showOfflineConfirmationModal || $showConflictModal || $showSetupModal || $syncMessage || $initialSyncComplete}
  <!-- This assumes components react to stores internally, but we need to mount the Container -->
{/if}
<SyncModalsContainer />
