...
export class DropboxProvider implements CloudProvider {
    ...
    private async refreshAccessToken(): Promise<string> {
        ...
        if (!refreshToken) throw new SyncError(SyncErrorCode.AUTH_ERROR, "No refresh token");
        ...
        const data = await res.json();
        if (data.error) throw new SyncError(SyncErrorCode.AUTH_ERROR);
        ...
        try {
            ...
            if (res.status === 401) {
                ...
                try {
                    ...
                    if (retry.status === 401) throw new SyncError(SyncErrorCode.AUTH_ERROR);
                    return retry;
                } catch {
                    throw new SyncError(SyncErrorCode.AUTH_ERROR);
                }
            }
            return res;
        } catch (e: any) {
            if (e instanceof SyncError) throw e;
            throw new SyncError(SyncErrorCode.NETWORK_ERROR, e.message);
        }
    }
